...
function dictValueParserSetDeps(): DictionaryValue<SetDeps> {
 ...
 type UserStablecoinWallet_init_args = {
    $$type: 'UserStablecoinWallet_init_args';
    master: Address;
    owner: Address;
}

function initUserStablecoinWallet_init_args(src: UserStablecoinWallet_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
        b_0.storeAddress(src.master);
        b_0.storeAddress(src.owner);
...
export class UserStablecoinWallet implements Contract {
    
    static async init(master: Address, owner: Address) {
        return await UserStablecoinWallet_init(master, owner);
    }
    
    static async fromInit(master: Address, owner: Address) {
        const init = await UserStablecoinWallet_init(master, owner);
        const address = contractAddress(0, init);
        return new UserStablecoinWallet(address, init);
    }
    
    static fromAddress(address: Address) {
        return new UserStablecoinWallet(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        errors: UserStablecoinWallet_errors
    };
